    ...
    providers:
      ...
      acme-gateway:
        ...
        compat:
          ...
          # Private OpenAI-compatible gateways often reject role: developer.
          # Hand-declared routes default this to false, so this is only needed
          # when the gateway explicitly supports the developer role.
...
How a thinking level travels — `reasoning_effort` alone, DeepSeek's `thinking: {type}` plus effort, z.ai's `thinking` object, and so on — is pi-ai's `compat.thinkingFormat`, which pi-ai guesses from the endpoint URL; a private gateway's URL says nothing, so a DeepSeek-dialect gateway would be spoken to in the OpenAI dialect with no way to correct it. `compat.thinkingFormat`, `compat.supportsReasoningEffort`, and `compat.supportsDeveloperRole` are configurable on the route (its models' default) and per model (winning per field), resolving model → route → installed catalog entry → pi-ai's URL-derived guess. For a hand-declared route, `supportsDeveloperRole` defaults to `false`: this preserves the older `system` role behavior for OpenAI-compatible gateways such as Console Go while keeping the reasoning-level controls. Set it to `true` only when the endpoint explicitly accepts `role: developer`. `thinkingFormat` accepts pi-ai's dispatchable formats except the two `chat-template` variants, which need `chatTemplateKwargs` this configuration does not expose. These switches exist only on `openai-completions` — the other protocols carry their reasoning shape in the protocol itself — so a model-level switch elsewhere fails resolution, a route-level one skips models of other protocols, and a route with no `openai-completions` model at all is refused. The rest of pi-ai's compat surface (`supportsStore`, `maxTokensField`, …) stays auto-detected and is deliberately not configurable here.

A model neither the entry nor the installed catalog sizes takes a context size inferred from its third-party model name when possible: suffixes such as `128k`, `131072`, and `1m` are recognized; large-context family names such as `max`, `flash`, `gemini`, `long-context`, and DeepSeek V4 receive a 1,048,576-token ceiling. An explicit per-model `contextWindow` wins, except that a third-party entry equal to the route's generic 262,144 fallback is treated as auto-discovered fallback when a stronger name hint exists; this lets stale `/models` metadata such as `deepseek-v4-flash: 262144` be corrected without hardcoding one model id. If no name gives a useful hint, the route's `defaultContextWindow` (262,144) and `defaultMaxTokens` (32,768) apply, so a listing that discloses nothing but ids still yields a serviceable route. These fallbacks are guesses by construction, which is why they are route fields a deployment can correct once rather than constants buried in the adapter; the resolved context sizes the model and never becomes a per-request cap.
...
A model that carries reasoning metadata — from the installed catalog or from its entry's `reasoningEfforts` — exposes pi-ai's ordered `getSupportedThinkingLevels(model)` result without filtering or normalization, including `off` and the model-specific availability of `xhigh` or `max`. A hand-declared third-party model gets the Codex-style five-level selector (`Off`, `Low`, `Medium`, `High`, `Max`) unless it explicitly sets `reasoningEfforts: false` or supplies its own map. The Harness exposes each canonical pi-ai level as an opaque ID; provider/model wire spellings remain inside pi-ai's `thinkingLevelMap`.

A model with no installed-catalog reasoning metadata uses the third-party default above unless its entry explicitly sets `reasoningEfforts: false`; a catalog model pi-ai marks as non-reasoning still exposes no `reasoning` capability. For third-party models, the adapter defaults the request to `off`, while an explicitly configured profile `reasoning` or per-request `GenerateOptions.reasoningEffort` takes precedence. A level absent from the exact model capability fails the request with `UNSUPPORTED_REASONING_EFFORT` before network I/O instead of being clamped. The Harness exposes each canonical pi-ai level as an opaque ID, and pi-ai keeps provider/model wire spellings inside `thinkingLevelMap`; its common stream options represent `off` by omitting `reasoning`.
