...
- **Permanent deletion.** `delete(id, signal?)` is serialized with writes and returns `true` only when a physical session artifact was removed. It supports the current `.jsonl.zstd`/`.jsonl` directory layout and the older flat-file layout, removes only empty derived directories, and never removes the configured root or another session.
