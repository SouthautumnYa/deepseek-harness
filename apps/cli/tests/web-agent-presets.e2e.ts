...
import { syncRoutingSuitePreset } from '../../../scripts/sync-routing-suite-preset.mjs'
...
async function bootWeb(
  ...
  const patches: PatchOptions[] = [
    ...
    // Both community Web extensions wait for the same server. The production
    // composition enables them; this test keeps the port-free boot isolated.
...
describe('the shipped Web composition', () => {
  ...
  it('supplies both shipped presets, and only those, from the system root', async () => {
    ...
    expect(listed.map(preset => preset.id).sort()).toEqual(['code', 'cordis', 'minimal', 'router-standard', 'standard'])
  ...
  it('composes the full agent from `standard`', async () => {
    ...
    try {
      ...
      expect(toolNames(ctx, handle.agent).filter(name => name !== 'glob' && name !== 'grep')).toEqual([
        'ask_user_question', STANDARD_SHELL_TOOL, 'create_goal', 'edit', 'exit_plan_mode',
