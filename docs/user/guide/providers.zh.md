...
![模型页：DeepSeek 卡片，以及添加提供方与添加第三方 API 两个入口](providers-models-page.zh.png)
...
对于公司网关、自建服务器或已安装目录中不存在的提供方，选择**添加第三方 API**。提供小写 Provider ID、基础 URL、API 协议、凭据和至少一个模型。大多数 OpenAI 兼容网关选择 **OpenAI 兼容（Chat Completions）**；Responses 和 Anthropic Messages 端点选择对应协议。
